...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 25-03-2026 a las 03:40:41
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insert_libro` (`c1_isbn` BIGINT(20), `c2_titulo` VARCHAR(255), `c3_genero` VARCHAR(20), `c4_paginas` INT(11), `c5diaspres` TINYINT(4))   INSERT INTO tbl_libro (
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_listaAutores` ()   SELECT aut_codigo, aut_apellido
FROM tbl_autor
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_tipoAutor` (`variable` VARCHAR(20))   SELECT aut_apellido AS 'Autor', tipoAutor
FROM tbl_autor
...
-- Funciones
--
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_contarTotalSocios` () RETURNS INT(11) DETERMINISTIC BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_DiasPrestamoLibro` (`p_isbn` BIGINT(20)) RETURNS INT(11) DETERMINISTIC BEGIN
...
--
DELIMITER $$
CREATE TRIGGER `auditoria_autor` AFTER UPDATE ON `tbl_autor` FOR EACH ROW BEGIN
    IF NOT (OLD.aut_muerte <=> NEW.aut_muerte) THEN
        INSERT INTO audi_autor (aut_codigo, campo_modificado, valor_anterior, valor_nuevo, usuario)
        VALUES (OLD.aut_codigo, 'aut_muerte', OLD.aut_muerte, NEW.aut_muerte, USER());
    END IF;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `autor_delete` BEFORE DELETE ON `tbl_autor` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `libro_insert` AFTER INSERT ON `tbl_libro` FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `update_Libro` BEFORE UPDATE ON `tbl_libro` FOR EACH ROW BEGIN
...
--
DELIMITER $$
CREATE TRIGGER `socio_delete` BEFORE DELETE ON `tbl_socio` FOR EACH ROW BEGIN
...
--
-- Estructura Stand-in para la vista `vista_autores_libros`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `vista_autores_libros` (
`aut_apellido` varchar(45)
,`lib_titulo` varchar(150)
);
...
--
-- Estructura Stand-in para la vista `vista_socio_prestamo_libro`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `vista_socio_prestamo_libro` (
`soc_numero` int(11)
,`soc_nombre` varchar(45)
,`pres_id` varchar(20)
,`lib_isbn` bigint(20)
...
--
-- Estructura Stand-in para la vista `vista_tipo_conautor`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `vista_tipo_conautor` (
`aut_codigo` int(11)
,`aut_apellido` varchar(45)
,`copiaAutor` int(11)
,`tipoAutor` varchar(10)
);

-- --------------------------------------------------------

--
-- Estructura para la vista `vista_autores_libros`
--
DROP TABLE IF EXISTS `vista_autores_libros`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_autores_libros`  AS SELECT `a`.`aut_apellido` AS `aut_apellido`, `l`.`lib_titulo` AS `lib_titulo` FROM ((`tbl_tipoautores` `ta` join `tbl_autor` `a` on(`ta`.`copiaAutor` = `a`.`aut_codigo`)) join `tbl_libro` `l` on(`ta`.`copiaisbn` = `l`.`lib_isbn`)) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_libros_prestados`  AS SELECT `l`.`lib_titulo` AS `lib_titulo`, `s`.`soc_nombre` AS `soc_nombre`, `s`.`soc_apellido` AS `soc_apellido`, `p`.`pres_fechaPrestamo` AS `pres_fechaPrestamo` FROM ((`tbl_prestamo` `p` join `tbl_libro` `l` on(`p`.`lib_copiaisbn` = `l`.`lib_isbn`)) join `tbl_socio` `s` on(`p`.`soc_copiaNumero` = `s`.`soc_numero`)) ;

-- --------------------------------------------------------

--
-- Estructura para la vista `vista_socio_prestamo_libro`
--
DROP TABLE IF EXISTS `vista_socio_prestamo_libro`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_socio_prestamo_libro`  AS SELECT `s`.`soc_numero` AS `soc_numero`, `s`.`soc_nombre` AS `soc_nombre`, `p`.`pres_id` AS `pres_id`, `l`.`lib_isbn` AS `lib_isbn`, `l`.`lib_titulo` AS `lib_titulo` FROM ((`tbl_socio` `s` join `tbl_prestamo` `p` on(`s`.`soc_numero` = `p`.`soc_copiaNumero`)) join `tbl_libro` `l` on(`p`.`lib_copiaisbn` = `l`.`lib_isbn`)) ;

-- --------------------------------------------------------

--
-- Estructura para la vista `vista_tipo_conautor`
--
DROP TABLE IF EXISTS `vista_tipo_conautor`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_tipo_conautor`  AS SELECT `a`.`aut_codigo` AS `aut_codigo`, `a`.`aut_apellido` AS `aut_apellido`, `t`.`copiaAutor` AS `copiaAutor`, `t`.`tipoAutor` AS `tipoAutor` FROM (`tbl_autor` `a` join `tbl_tipoautores` `t` on(`a`.`aut_codigo` = `t`.`copiaAutor`)) ;
...
-- Eventos
--
CREATE DEFINER=`root`@`localhost` EVENT `eliminar_prestamos_vencidos` ON SCHEDULE EVERY 1 DAY STARTS '2026-03-10 00:00:00' ENDS '2026-12-31 23:59:59' ON COMPLETION NOT PRESERVE ENABLE DO DELETE FROM tbl_prestamo
